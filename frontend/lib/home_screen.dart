...
class _HomeScreenState extends State<HomeScreen> {
    ...
    if (online) {
      try {
        ...
        await Future.wait(remoteSets.map((set) async {
          ...
          final offlineFlashcards = (fullSet['cards'] as List).map((card) {
  ...
  void _loadFromHive(Box<OfflineFlashcardSet> box) {
    final local = box.values.toList()
      ..sort((a, b) => b.createdAt.compareTo(a.createdAt)); // zoradenie zostupne
                            ...
                            if (mounted) {
                              await showDialog(
                                ...
                                builder: (ctx) => AlertDialog(
                                  ...
                                  content: const Text('You are offline – creating a set requires internet connection.'),
                                  actions: [
                                    TextButton(
